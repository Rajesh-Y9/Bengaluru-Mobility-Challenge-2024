...
from numba.core.registry import CPUDispatcher  # Correct import for CPUDispatcher

def _find_available_functions(module_name):
    all_members = getmembers(module_name)
    available_functions = [member[0] for member in all_members if isinstance(member[1], CPUDispatcher)]
...
def hausdorff_distance(XA, XB, distance='euclidean'):
    ...
    assert np.issubdtype(XA.dtype, np.number) and np.issubdtype(XB.dtype, np.number), \
